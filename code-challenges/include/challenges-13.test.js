...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(el => el.substring(1,4)+el.substring(6, 9)+el.substring(10, 14));
...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.filter(el => el.includes(target));
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(el => el.filter(e => !e.includes('Brook')));
