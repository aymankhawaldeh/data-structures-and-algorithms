...
const count = (target, input) => {
    return input.map( elemArr => elemArr.filter( elem => elem===target)).reduce((acc,val) => acc+=val.length,0);
  }
...
const totalSum = (input) => {
	return input.reduce( (acc,val) => acc + val.reduce( (acc2,val2) => acc2+val2 ) ,0 )
};
...
const divisibleByFiveTwoToThePower = (input) => {
    return input.map( (val) => val.filter( (val2) => (typeof val2 === 'number' && val2%5 === 0)).map( val3 => Math.pow(2,val3)))
  };
